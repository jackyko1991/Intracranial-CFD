...
def normalizeVessels(case_dir,dist_from_bif_inlet = 35,dist_from_bif_outlet = 35):
...
def normalizeVessels2(case_dir,dist_from_bif_inlet = 35,dist_from_bif_outlet = 35):
	# phases = ["baseline","baseline-post","12months","followup"]
	phases = ["baseline"]
	...
	thresholdFilter.SetInputArrayToProcess(0, 0, 0, vtk.vtkDataObject.FIELD_ASSOCIATION_POINTS, "Abscissas")
	thresholdFilter.Update()

	vtpWriter = vtk.vtkXMLPolyDataWriter()
	vtpWriter.SetFileName(os.path.join(case_dir,phase,"centerline_clipped.vtp"))
...
def main():
	...
	capping = False
	stl_concat = False
	batch_post_clip_centerline = False
	...
	ignore_case = []
	# ignore_case = ["ChanPitChuen"]

	for sub_data_folder in sub_data_folders:
		dataList = os.listdir(os.path.join(data_dir,sub_data_folder))
		...
		for case in pbar:
			...
			for phase in phases:
				if batch_centerline:
					if phase == "followup":
						# continue
						centerlineCalculation(os.path.join(data_dir, sub_data_folder,case,phase),relaxation=smooth_relaxation_CBCT)
					else:
						# continue
						centerlineCalculation(os.path.join(data_dir, sub_data_folder,case,phase),relaxation=smooth_relaxation_3DRA)

			if batch_clip:
				normalizeVessels2(os.path.join(data_dir, sub_data_folder,case))

			for phase in phases:
				if batch_post_clip_centerline:
					if phase == "followup":
						# continue
						centerlineCalculation(os.path.join(data_dir,case,phase),relaxation=smooth_relaxation_CBCT,post=True)
					else:
						# continue
						centerlineCalculation(os.path.join(data_dir,case,phase),relaxation=smooth_relaxation_3DRA,post=True)
